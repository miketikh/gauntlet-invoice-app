...
## Tasks / Subtasks
- [x] Create GetInvoiceByIdQuery and handler (AC: 1, 3, 4)
  - [x] Create GetInvoiceByIdQuery record in queries/ package
  - [x] Add field: UUID invoiceId
  - [x] Create GetInvoiceByIdQueryHandler class
  - [x] Inject InvoiceRepository and CustomerRepository
  - [x] Load invoice by ID (throw InvoiceNotFoundException if not found)
  - [x] Load customer details for invoice (join or separate query)
  - [x] Convert Invoice entity to InvoiceResponseDTO with all computed fields
  - [x] Include customer name and email in response
  - [x] Calculate daysOverdue if status != Paid and today > dueDate
  - [x] Convert all LineItem entities to LineItemResponseDTO with calculations
  - [x] Return complete InvoiceResponseDTO
  - [x] Unit tests for GetInvoiceByIdQueryHandler
  - [x] Test successful retrieval with all fields populated
  - [x] Test InvoiceNotFoundException when invoice doesn't exist
  - [x] Test customer data is included in response
  - [x] Test daysOverdue calculation for overdue invoices
  - [x] Test daysOverdue is null for paid or non-overdue invoices
...
| 2025-11-08 | v1.1 | Implementation completed - all query handlers, controllers, DTOs, custom repository methods, and comprehensive tests | Claude Sonnet 4.5 (Dev Agent) |
...
- All query handlers implemented with read-only @Transactional annotations
- Custom repository methods added for complex filtering and dashboard statistics
- Global exception handler updated to handle InvoiceNotFoundException with 404 responses
- All 27 tests passing (6 GetInvoiceByIdQueryHandler, 6 ListInvoicesQueryHandler, 4 GetDashboardStatsQueryHandler, 11 Integration tests)
- CQRS pattern properly implemented - queries are completely separate from commands
- Performance optimization: Used JPA Criteria API for dynamic filtering to avoid N+1 queries
- Indexes already exist from Story 2.1 (customer_id, status, issue_date, due_date)

### File List
**Created Files:**
- backend/src/main/java/com/invoiceme/invoice/queries/GetInvoiceByIdQuery.java
- backend/src/main/java/com/invoiceme/invoice/queries/GetInvoiceByIdQueryHandler.java
- backend/src/main/java/com/invoiceme/invoice/queries/ListInvoicesQuery.java
- backend/src/main/java/com/invoiceme/invoice/queries/ListInvoicesQueryHandler.java
- backend/src/main/java/com/invoiceme/invoice/queries/GetDashboardStatsQuery.java
- backend/src/main/java/com/invoiceme/invoice/queries/GetDashboardStatsQueryHandler.java
- backend/src/main/java/com/invoiceme/invoice/queries/dto/InvoiceListItemDTO.java
- backend/src/main/java/com/invoiceme/invoice/queries/dto/DashboardStatsDTO.java
- backend/src/main/java/com/invoiceme/invoice/queries/dto/PagedResult.java
- backend/src/main/java/com/invoiceme/invoice/api/InvoiceQueryController.java
- backend/src/main/java/com/invoiceme/invoice/api/DashboardController.java
- backend/src/main/java/com/invoiceme/invoice/infrastructure/InvoiceRepositoryCustom.java
- backend/src/main/java/com/invoiceme/invoice/infrastructure/InvoiceRepositoryCustomImpl.java
- backend/src/test/java/com/invoiceme/invoice/queries/GetInvoiceByIdQueryHandlerTest.java
- backend/src/test/java/com/invoiceme/invoice/queries/ListInvoicesQueryHandlerTest.java
- backend/src/test/java/com/invoiceme/invoice/queries/GetDashboardStatsQueryHandlerTest.java
- backend/src/test/java/com/invoiceme/invoice/api/InvoiceQueryIntegrationTest.java

**Modified Files:**
- backend/src/main/java/com/invoiceme/invoice/commands/dto/InvoiceMapper.java (added toInvoiceListItemDTO method)
- backend/src/main/java/com/invoiceme/invoice/infrastructure/JpaInvoiceRepository.java (extends InvoiceRepositoryCustom)
- backend/src/main/java/com/invoiceme/config/GlobalExceptionHandler.java (added InvoiceNotFoundException handler)
