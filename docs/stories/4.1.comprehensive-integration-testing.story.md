...
## Tasks / Subtasks
- [x] Set up integration test infrastructure (AC: 3, 4)
  - [x] Add TestContainers dependency to backend/pom.xml:
    ```xml
    <dependency>
    ...
    ```
  - [x] Create base test class: backend/src/test/java/com/invoiceme/IntegrationTestBase.java
  - [x] Configure @SpringBootTest with random port
  - [x] Configure @Testcontainers with PostgreSQL container
  - [x] Set up TestRestTemplate or WebTestClient for API calls
  - [x] Create test database initialization scripts
  - [x] Configure test application.yml with TestContainers properties
  - [x] Add @BeforeEach method to reset database state
  - [x] Add helper methods for JWT token generation in tests
  - [x] Configure test logging to show SQL statements (for debugging)

- [x] Create test data builders (AC: 2)
  - [x] Create CustomerTestBuilder.java in backend/src/test/java/com/invoiceme/builders/
    ```java
    ...
    ```
  - [x] Create InvoiceTestBuilder.java:
  ...
  - [x] Create LineItemTestBuilder.java:
  ...
  - [x] Create PaymentTestBuilder.java:
  ...
  - [x] Add utility methods to builders:
...
- [x] Add test coverage reporting (AC: 8)
  - [x] Add JaCoCo plugin to backend/pom.xml:
    ```xml
    <plugin>
...
- [x] Create test utilities and helpers (AC: 1-8)
  - [x] Create JwtTestHelper.java:
  ...
  - [ ] Create DatabaseTestHelper.java:
    - clearAllTables() - reset database state (DONE in IntegrationTestBase)
    - createTestCustomer() - quick customer creation (DONE via builders)
    - createTestInvoice() - quick invoice creation (DONE via builders)
    - createTestPayment() - quick payment creation (DONE via builders)
...
| 2025-11-09 | v1.1 | Set up integration test infrastructure, test builders, JaCoCo plugin | James (Dev Agent) |
...
None - Initial implementation phase completed successfully

### Completion Notes List
- Successfully set up TestContainers infrastructure with PostgreSQL 15
- Created IntegrationTestBase with proper configuration for all integration tests
- Implemented all test data builders (Customer, Invoice, LineItem, Payment) with full builder pattern support
- Added JaCoCo plugin for code coverage tracking with 80% threshold
- JwtTestHelper created for authentication in integration tests
- All infrastructure code compiles successfully with mvn clean compile test-compile
- Ready to implement actual integration test classes

### File List

**New Files Created:**
- `/Users/mike/gauntlet/invoice-me/backend/src/test/java/com/invoiceme/IntegrationTestBase.java` - Base class for all integration tests
- `/Users/mike/gauntlet/invoice-me/backend/src/test/resources/application-integration-test.properties` - Integration test configuration
- `/Users/mike/gauntlet/invoice-me/backend/src/test/java/com/invoiceme/helpers/JwtTestHelper.java` - JWT token helper for tests
- `/Users/mike/gauntlet/invoice-me/backend/src/test/java/com/invoiceme/builders/CustomerTestBuilder.java` - Customer test data builder
- `/Users/mike/gauntlet/invoice-me/backend/src/test/java/com/invoiceme/builders/InvoiceTestBuilder.java` - Invoice test data builder
- `/Users/mike/gauntlet/invoice-me/backend/src/test/java/com/invoiceme/builders/LineItemTestBuilder.java` - LineItem test data builder
- `/Users/mike/gauntlet/invoice-me/backend/src/test/java/com/invoiceme/builders/PaymentTestBuilder.java` - Payment test data builder

**Modified Files:**
- `/Users/mike/gauntlet/invoice-me/backend/pom.xml` - Added testcontainers junit-jupiter dependency and JaCoCo plugin configuration
