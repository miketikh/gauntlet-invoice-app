...
## Tasks / Subtasks
- [ ] Create LineItem value object (AC: 2)
  - [ ] Create LineItem.java as immutable value object in domain/
  - [ ] Add fields: id, description, quantity, unitPrice, discountPercent, taxRate
  - [ ] Add calculated fields: subtotal, discountAmount, taxableAmount, taxAmount, total
  - [ ] Implement calculation methods for line item totals
  - [ ] Implement equals() and hashCode() based on business identity
  - [ ] Add validation: quantity > 0, unitPrice >= 0, discountPercent 0-1, taxRate >= 0
  - [ ] Make LineItem immutable (all fields final, no setters)

- [ ] Create Invoice aggregate root (AC: 1, 3, 4)
  - [ ] Create Invoice.java as aggregate root in domain/
  - [ ] Add basic fields: id, invoiceNumber, customerId, issueDate, dueDate, status, paymentTerms, notes
  - [ ] Add calculated fields: subtotal, totalDiscount, totalTax, totalAmount, balance
  - [ ] Add lineItems collection (List<LineItem>)
  - [ ] Add temporal fields: createdAt, updatedAt
  - [ ] Implement InvoiceStatus enum (Draft, Sent, Paid)
  - [ ] Implement addLineItem() method with invariant checks
  - [ ] Implement removeLineItem() method with invariant checks
  - [ ] Implement updateLineItem() method with invariant checks
  - [ ] Implement calculateTotals() private method to recalculate all totals
  - [ ] Enforce invariant: line items cannot be modified if status != Draft
  - [ ] Enforce invariant: must have at least one line item to send
  - [ ] Add domain validation: dueDate must be >= issueDate

- [ ] Implement Invoice state transition methods (AC: 4, 6)
  - [ ] Create markAsSent() method with business rule validation
  - [ ] Create markAsPaid() method with balance check validation
  - [ ] Implement applyPayment(amount) method to reduce balance
  - [ ] Prevent state transitions that violate business rules
  - [ ] Publish InvoiceStatusChanged event on state transitions
  - [ ] Add canBeSent() validation method (checks for line items)
  - [ ] Add canBePaid() validation method (checks status and balance)

- [ ] Create Invoice domain events (AC: 6)
  - [ ] Create domain/events/ package
  - [ ] Create InvoiceCreated event with invoice ID and customer ID
  - [ ] Create InvoiceStatusChanged event with old/new status
  - [ ] Create LineItemAdded event with invoice ID and line item
  - [ ] Create LineItemRemoved event with invoice ID and line item ID
  - [ ] Implement base DomainEvent interface/record if not exists
  - [ ] Add event publishing mechanism to aggregate methods

- [ ] Implement Invoice number generation (AC: 8)
  - [ ] Create InvoiceNumberGenerator service in domain/
  - [ ] Implement sequential number generation: INV-{YEAR}-{SEQUENCE}
  - [ ] Create database sequence for invoice numbers
  - [ ] Ensure thread-safe sequential generation
  - [ ] Add database migration for invoice_number_seq
  - [ ] Create generateNextInvoiceNumber() method
  - [ ] Integrate with Invoice creation process

- [ ] Create InvoiceRepository interface (AC: 5)
  - [ ] Define InvoiceRepository interface in domain/
  - [ ] Add method: save(invoice) - persist aggregate
  - [ ] Add method: findById(id) - retrieve aggregate with line items
  - [ ] Add method: existsByInvoiceNumber(invoiceNumber) - uniqueness check
  - [ ] Add method: getNextInvoiceNumber() - for number generation
  - [ ] Document aggregate persistence requirements (cascade line items)
  - [ ] Document transactional boundaries

- [ ] Implement JPA persistence for Invoice aggregate (AC: 5)
  - [ ] Create JpaInvoiceRepository in infrastructure/
  - [ ] Implement Spring Data JPA repository extending JpaRepository
  - [ ] Configure JPA entity mapping for Invoice
  - [ ] Configure JPA embedded/JSON mapping for LineItem collection
  - [ ] Set up cascade operations (persist, merge line items with invoice)
  - [ ] Implement soft-delete support if needed
  - [ ] Add database indexes: customerId, status, invoiceNumber, dueDate
  - [ ] Create migration script: V4__create_invoices_table.sql

- [ ] Create database schema for invoices (AC: 5)
  - [ ] Create invoices table with all fields
  - [ ] Add JSONB column for line_items (PostgreSQL) or TEXT for JSON
  - [ ] Add foreign key: customerId references customers(id)
  - [ ] Add unique constraint on invoice_number
  - [ ] Add check constraint: status IN ('Draft', 'Sent', 'Paid')
  - [ ] Add check constraint: balance >= 0
  - [ ] Add check constraint: totalAmount >= 0
  - [ ] Create indexes for query performance
  - [ ] Create invoice_number_seq sequence
  - [ ] Create generate_invoice_number() database function

- [ ] Write comprehensive domain tests (AC: 7)
  - [ ] Unit tests for LineItem value object validation and calculations
  - [ ] Unit tests for Invoice aggregate creation
  - [ ] Unit tests for addLineItem() with invariant enforcement
  - [ ] Unit tests for removeLineItem() with invariant enforcement
  - [ ] Unit tests for updateLineItem() with invariant enforcement
  - [ ] Unit tests for calculateTotals() accuracy
  - [ ] Unit tests for markAsSent() state transition and validations
  - [ ] Unit tests for markAsPaid() state transition
  - [ ] Unit tests for applyPayment() balance updates
  - [ ] Unit tests for all domain invariant violations (expect exceptions)
  - [ ] Unit tests for InvoiceNumberGenerator sequence generation
  - [ ] Integration tests for repository aggregate persistence
  - [ ] Test that line items are persisted and retrieved correctly
